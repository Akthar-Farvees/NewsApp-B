...
${
  article.content && article.content.length > 1000
    ? "**Full Content:**\n" +
  ...
  async summarizeArticle(request: SummarizationRequest): Promise<string> {
    try {
      ...
      switch (request.mode) {
        case "short":
          promptInstructions =
            "Provide a concise 1-2 sentence summary highlighting the main point.";
          break;
        case "medium":
          promptInstructions =
            "Provide a 3-4 sentence summary covering the key points and implications.";
          break;
        case "long":
          promptInstructions =
            "Provide a detailed paragraph summary with bullet points for key takeaways and potential impact.";
          break;
      }
      ...
      await this.saveChatMessage(request.userId, request.articleId, [
        {
          role: "user",
          content: `Summarize this article (${request.mode} mode)`,
          timestamp: Date.now(),
        },
  ...
  async compareArticles(articleIds: string[], userId: string): Promise<string> {
    try {
      if (!openai) {
        return "AI comparison is unavailable. Please provide an API key.";
      }
      ...
      const articleContexts = validArticles
        .map(
          (article, index) =>
      ...
      return (
        response.choices[0].message.content || "Unable to generate comparison"
      );
    } catch (error) {
